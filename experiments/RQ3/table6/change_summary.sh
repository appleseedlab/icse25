...
while IFS=, read -r commit configfile repairedconfig; do
	...
	changes=$(python3 $SCRIPT_DIR/measure_change.py --original-config ${syzbot_configuration_files_dir}/${configfile} ${repaired_configuration_files_dir}/${repairedconfig} 2>/dev/null | jq .repaired[].change_wrt_original | paste -sd+ | bc -lq)
	echo -n ${configfile},
