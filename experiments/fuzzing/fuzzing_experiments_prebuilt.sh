...
function extract_kernel_image() {
    local kernel_image_path="$1"
    local dir_linux_next="$2"

    # Extract vmlinux and bzImage from kernel_image_path
    # First, create a temporary directory to extract the kernel image
    tmp_dir=$(mktemp -d)
    echo "[+] Extracting vmlinux and bzImage from $kernel_image_path..."
    7z x "$kernel_image_path" -o"$tmp_dir" || {
        echo "[-] 7z extraction failed for $kernel_image_path"
        exit 1
    }
    # Move the extracted files to the linux-next directory
    mv "$tmp_dir/vmlinux" "$dir_linux_next/vmlinux"
    mv "$tmp_dir/bzImage" "$dir_linux_next/arch/x86/boot/bzImage"
    rm -rf "$tmp_dir"
}
...
while IFS=, read -r commit_hash syzbot_config_name git_tag repaired_config_name kernel_image_path; do
    ...
    extract_kernel_image "$kernel_image_path" "$dir_linux_next"
