...
We first determine at what source file the bug occurred from the syzkaller bug report.
Next, we provide the file name to the script called ./scripts/get_mainter.pl to obtain the email addresses of the maintainers of the file.
Then, we generate a bug report and include the head commit ID of the Linux-next version we fuzzed, link to the kernel configuration file used to build the kernel, a reproducer file (if exists) to reproduce the crash, and syzkaller bug report of the crash.
