...
while IFS=, read -r config_filename commit_id tag; do
  ...
  rsync -avm --include='*/' --include='*.i' --exclude='*' --prune-empty-dirs --relative "$LINUX_REPO_DIR" /home/sanan/linux-next-$config_filename-$tag-repaired
  ...
  rsync -avm --include='*/' --include='*_source_lines.txt' --exclude='*' --prune-empty-dirs --relative "$LINUX_REPO_DIR" /home/sanan/linux-next-$config_filename-$tag-repaired
