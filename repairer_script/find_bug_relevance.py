...
def find_function_definitions(functions, src_dir):
    ...
    for function in functions:
        # Constructing the cscope query command
        ...
        if result.returncode == 0 and result.stdout:
            ...
            function_defs[function] = file_names
        else:
            function_defs[function] = None  # No definition found or error occurred
...
def main():
    ...
    src_dir = "/home/sanan/linux"  # Replace with the actual path
    ...
    all_config_options = {}
    for function, files in function_defs.items():
        if files:
            for file in files:
                if file.endswith('.c') or file.endswith('.o'):
                    print(f"Processing {file} for function {function}")
                    config_options = find_kernel_config_for_file(args, file, klocalizer_path)
                    if config_options:
                        all_config_options[file] = config_options
                    else:
                        print(f"No configuration options found for {file}")
